...
class RideTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.app.post("/api/v1/auth/register",
                      data=json.dumps(dict(email="daniel@live.com", username="daniel",
                                           password="12345678")),
        ...
        #User 2
        self.app.post("/api/v1/auth/register",
                      data=json.dumps(dict(email="ken@live.com", username="ken",
                                           password="12345678")),
                      content_type="application/json")
        self.login_user2 = self.app.post("/api/v1/auth/login",
                                         data=json.dumps(dict(username="ken",
                                                              password="12345678")),
                                         content_type="application/json")
        ...
        self.app.post("/api/v1/rides",
                      data=json.dumps(dict(category="SUV", pick_up="Karen",
                                           drop_off="CBD", date_time="19th June 11.00 A.M", price="Ksh.800")),
                      headers={
                          "Authorization": "Bearer {}".format(self.access_token),
                          "Content-Type": "application/json"
                          })

        self.dict = dict(category="limousine", pick_up="Uthiru", drop_off="Nairobi",
    ...
    def test_category_empty(self):
        ...
        response = self.app.post("/api/v1/rides",
                                 data=json.dumps(dict(category="", pick_up="UpperHill",
                                                      drop_off="TRM", date_time="21st June 15.00", price="Ksh. 500")),
                                 headers={
                                     "Authorization": "Bearer {}".format(self.access_token),
                                     "Content-Type": "application/json"
                                     })
    ...
    def test_pick_up_empty(self):
        ...
        response = self.app.post("/api/v1/rides",
                                 data=json.dumps(dict(category="SUV", pick_up="",
                                                      drop_off="CBD", date_time="19th June 11.00 A.M", price="Ksh. 800")),
                                 headers={
                                     "Authorization": "Bearer {}".format(self.access_token),
                                     "Content-Type": "application/json"
        ...
        self.assertEqual(response.status_code, 406)
        response_msg = json.loads(response.data.decode("UTF-8"))
        self.assertEqual(response_msg['pick_up-Error:']["message"], "pick_up cannot be an empty string")

    def test_drop_off_empty(self):
        '''Test for blank drop_off location'''
        response = self.app.post("/api/v1/rides",
                                 data=json.dumps(dict(category="limousine", pick_up="CBD",
                                                      drop_off="", date_time="20th June 10.00 a.m", price="Ksh. 700")),
                                 headers={
                                     "Authorization": "Bearer {}".format(self.access_token),
                                     "Content-Type": "application/json"
                                     })
        ...
        self.assertEqual(response.status_code, 406)
        response_msg = json.loads(response.data.decode("UTF-8"))
        self.assertEqual(response_msg['drop_off-Error:']["message"], "drop_off cannot be an empty string")
